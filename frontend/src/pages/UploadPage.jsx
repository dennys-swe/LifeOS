...
export default function UploadPage({ onNavigate }) {
  ...
  const statusColor = useMemo(() => {
    if (status === STATUS.success) return "text-emerald-600 dark:text-emerald-400";
    if (status === STATUS.error) return "text-rose-600 dark:text-rose-400";
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-8 px-6 py-10">
      <header className="flex flex-col gap-3">
        <p className="text-xs uppercase tracking-[0.4em] text-gray-400 dark:text-slate-500">Importação de Extratos</p>
        <h1 className="text-3xl font-semibold text-gray-900 dark:text-white md:text-4xl">Enviar arquivo CSV</h1>
      ...
      <section
        className={`flex flex-col items-center justify-center gap-4 rounded-3xl border-2 border-dashed px-6 py-12 text-center shadow-lg transition ${
          selectedFile ? "border-emerald-400 bg-emerald-500/10" : "border-emerald-500/50 bg-gray-50 dark:bg-slate-900/70"
        }`}
        onDrop={handleDrop}
      ...
      >
        ...
        <div>
          <p className="text-lg font-semibold text-gray-900 dark:text-white">Clique ou arraste</p>
          <p className="text-sm text-gray-500 dark:text-slate-400">CSV exportado do seu banco</p>
        </div>
        ...
        {fileName && <p className="text-xs text-gray-500 dark:text-slate-400">Arquivo: {fileName}</p>}
      </section>

      <section className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900/70">
        ...
        {status === STATUS.success && suggestions.length === 0 && onNavigate && (
          <button type="button" onClick={() => onNavigate(PAGES.dashboard)}
            className="mt-3 rounded-full border border-emerald-500/60 px-4 py-2 text-sm font-semibold text-emerald-600 hover:bg-emerald-500/10 dark:text-emerald-300">
            Ir para o Dashboard
          </button>
        )}
      </section>
      ...
      {suggestions.length > 0 && (
        <section className="flex flex-col gap-4">
          ...
          {suggestions.map((s) => (
            <div key={s.payable_id} className="flex flex-col gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-slate-800 dark:bg-slate-950/60 md:flex-row md:items-center md:justify-between">
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-gray-400 dark:text-slate-500">
                  <span>Transação</span>
                </div>
                <p className="text-sm text-gray-900 dark:text-white">{s.transaction_description}</p>
                <p className="text-xs text-gray-500 dark:text-slate-400">R$ {Number(s.transaction_amount).toFixed(2)}</p>
              </div>
              ...
              <div className="flex flex-col items-center gap-1">
                ...
                <span className="text-xs text-gray-400 dark:text-slate-500">{Math.round(s.confidence_score * 100)}% match</span>
              </div>
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-sm text-gray-900 dark:text-white">{s.payable_title}</p>
                <p className="text-xs text-gray-500 dark:text-slate-400">R$ {Number(s.payable_amount).toFixed(2)}</p>
              </div>

              <div className="flex gap-2">
                <button type="button" disabled={confirming.has(s.payable_id)} onClick={() => handleConfirmSuggestion(s)}
                  className="rounded-full border border-emerald-500/40 px-3 py-1 text-xs text-emerald-600 transition hover:bg-emerald-500/10 disabled:opacity-50 dark:text-emerald-300">
                  {confirming.has(s.payable_id) ? "..." : "Confirmar"}
                </button>
                <button type="button" onClick={() => handleIgnoreSuggestion(s.payable_id)}
                  className="rounded-full border border-gray-200 px-3 py-1 text-xs text-gray-600 hover:bg-gray-100 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-900">
                  Ignorar
                </button>
              </div>
            </div>
