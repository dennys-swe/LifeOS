...
- **`creditCardMetadata.billForecastDate`** (ex: `"2026-08"`) é a fonte mais confiável de qual competência a pendência vai cair — mas o Itaú **erra**: (a) lançamentos antigos vêm rotulados pelo mês da compra com a fatura já paga → rolam para o ciclo seguinte; (b) **compras do próprio mês-alvo vêm rotuladas com o mês seguinte** (compra de 05/09 como `"2026-10"`), inflando outubro e esvaziando setembro (~R$ 226 de diferença medidos). `_bill_month` trata: se a pendência tem `billForecastDate` à frente do alvo mas a **data da compra** cai no mês-alvo, a data manda. Ver `tests/test_open_bill.py::test_itau_forecast_one_month_ahead...` e a fixture `_synthetic-itau`.
