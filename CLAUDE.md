...
**Deploy de migration:** o Build Command do Render é `pip install -r requirements.txt && alembic upgrade head` (configurado no dashboard, não no `render.yaml`), então **toda migration mergeada na `main` é aplicada no deploy**. Antes disso o build só instalava dependências e uma migration precisava ser rodada à mão — um PR com migration derrubava produção (o modelo referenciava uma coluna que não existia). Para aplicar uma migration em produção fora do deploy: `cd backend && ALLOW_PROD_DB=1 alembic upgrade head`.
...
- **`creditCardMetadata.billId` bate exato com o `id` de uma fatura da Bills API** — inclusive fatura **futura só projetada** (confirmado contra captura real de Inter/Itaú/Luiza, issue #85). É o sinal mais forte que existe, mais confiável que `billForecastDate`: o Inter **nunca** manda `billForecastDate`, mas manda `billId` até em transação recém-postada, o que permite achar a fatura-alvo certa mesmo sem forecast. `open_bill_service._bill_month` usa isso só como **confirmação positiva** (se `billId` bate com o `id` da fatura-alvo, conta; se não bate, cai na leitura de forecast/data de sempre) — nunca para excluir, porque a mesma transação comum já cobrada de verdade também carrega `billId` (da fatura fechada), e forçar exclusão por isso já causou contagem em dobro numa tentativa anterior (revertida, ver histórico da issue #85).
