...
def test_counts_future_dated_installment_without_projecting_it_twice():
    ...
    txs = [
        _tx(
            15.99, "2026-08-10", description="ANUIDADE 02/12", installment=2, total_installments=12
        ),
        _tx(
            15.99, "2026-09-10", description="ANUIDADE 03/12", installment=3, total_installments=12
        ),
    ]
    assert open_bill_service.compute_open_bill_amount(
...
def test_ignores_installment_already_charged_in_closed_bill():
    ...
    txs = [
        _tx(
            ...
            bill_id="bill-jul",
            status="POSTED",
            description="PIX MARIA 02/02",
            installment=2,
            total_installments=2,
        ),
        _tx(
            29.82, "2026-07-10", description="PIX MARIA 02/02", installment=2, total_installments=2
        ),
    ]
    assert open_bill_service.compute_open_bill_amount(
        txs, date(2026, 8, 10), date(2026, 7, 10)
...
def test_closed_bill_replaces_open_estimate_and_generates_payable(db_session, user):
    ...
    assert (
        db_session.query(CreditCardBill).filter(CreditCardBill.due_date == date(2026, 8, 8)).count()
        == 1
    )
