...
def test_pluggy_webhook_ignores_unknown_item(client):
    r = client.post(
        "/webhooks/pluggy", json={"event": "transactions/created", "itemId": str(uuid4())}
    )
    assert r.status_code == 200
...
@patch("app.api.endpoints.webhooks.bank_sync_service.sync_account")
def test_secret_path_processes_with_correct_secret(
    mock_sync, client, db_session, user, monkeypatch
):
    ...
    r = client.post(
        "/webhooks/pluggy/s3cr3t", json={"event": "transactions/created", "itemId": item_id}
    )
...
def test_secret_path_rejects_wrong_secret(mock_sync, client, db_session, user, monkeypatch):
    ...
    r = client.post(
        "/webhooks/pluggy/wrong", json={"event": "transactions/created", "itemId": item_id}
    )
...
def test_secret_path_lenient_when_no_secret_configured(mock_sync, client, db_session, user):
    ...
    r = client.post(
        "/webhooks/pluggy/anything", json={"event": "transactions/created", "itemId": item_id}
    )
