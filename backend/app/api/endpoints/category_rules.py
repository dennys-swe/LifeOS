...
def create_category_rule(payload: CategoryRuleCreate, db: Session = Depends(get_db)):
    return category_rule_service.create_rule(db, payload)


@router.delete("/{rule_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_category_rule(rule_id: UUID, db: Session = Depends(get_db)):
    rule = category_rule_service.get_rule(db, rule_id)
    if rule is None:
