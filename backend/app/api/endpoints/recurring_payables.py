...
from app.services import recurring_detection_service, recurring_service
...
# /suggestions must be registered before /{recurring_id} to avoid FastAPI treating
# "suggestions" as UUID
...
def get_recurring_suggestions(
    ...
    return recurring_detection_service.detect_recurring_candidates(db, user.id)
...
def generate_payables_for_month(
    ...
    return recurring_service.generate_for_month(db, user.id, month=month, year=year)
...
def create_recurring_payable(
    ...
    return recurring_service.create_recurring(db, user.id, payload)


@router.get("/{recurring_id}", response_model=RecurringPayableResponse)
def get_recurring_payable(
    recurring_id: UUID,
    db: Session = Depends(get_db),
    user: User = Depends(current_active_user),
):
    rec = recurring_service.get_recurring(db, user.id, recurring_id)
    if rec is None:
