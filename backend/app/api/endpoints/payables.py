...
def create_payable(
    ...
    return payable_service.create_payable(db, user.id, payload)
...
def list_payables(
    ...
    return payable_service.list_payables(db, user.id, month=month, year=year)
...
def update_payable(
    ...
    payable = payable_service.get_payable(db, user.id, payable_id)
    if payable is None:
...
def reconcile_payable(
    ...
    try:
        return confirm_reconciliation(
            db, user.id, transaction_id=transaction_id, payable_id=payable_id
        )
